...
class Sessions extends Model
{
    ...
    public function start(Users $user, string $sessionId, string $token, string $userIp, int $pageId) : Users
    {
        ...
        if ($banInfo) {
            if ($banInfo['ip'] || $banInfo['users_id'] || $banInfo['email']) {
                throw new ForbiddenException(_('This account has been banned. Please contact the administrators.'));
            }
        }
    ...
    public function check(Users $user, string $sessionId, string $userIp, int $pageId) : Users
    {
        ...
        $sql = "SELECT user.*, session.*
                FROM Canvas\Models\Sessions session, Canvas\Models\Users user
        ...
        if ($userData->user) {
            ...
            if ($currentTime - $userData->session->time > 60) {
                ...
                if (!$session->update()) {
                    throw new ForbiddenException(current($session->getMessages()));
                }
        ...
        throw new ForbiddenException(_('No Session Token Found'));
    }
